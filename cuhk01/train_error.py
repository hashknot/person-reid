...
tf.app.flags.DEFINE_string('eval_dir', 'out/eval',
...
tf.app.flags.DEFINE_integer('num_examples', constants.BATCH_SIZE,
                            """Number of examples to run.""")
tf.app.flags.DEFINE_boolean('run_once', constants.EVAL_RUN_ONCE,
                            """Whether to run eval only once.""")

def get_accuracy(sess, num_examples, batch_size, ):
...
def eval_once(saver, summary_writer, top_k_op, summary_op):
    ...
    with tf.Session() as sess:
        ...
        try:
            ...
            # Compute precision @ 1.
            precision = true_count / total_sample_count
            print('%s: Training accuracy @ 1 = %.3f' % (datetime.now(), precision))

            summary = tf.Summary()
            summary.ParseFromString(sess.run(summary_op))
            summary.value.add(tag='Training accuracy', simple_value=precision)
